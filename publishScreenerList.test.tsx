...
describe("draft-screener-list", () => {
  ...
  test(" tab switching", async () => {
    ...
    const publishTab = screen.getByRole("Published-role"); // Tab action for draft intake list
    expect(publishTab).toBeInTheDocument();
    await act(() =>
      waitFor(() => {
        fireEvent.click(publishTab); // Clicking the action to swicth to draft intake screener list
      }),
    );
    const updatedTitle = screen.getByText("Published Screeners List", { selector: "h2" }); // Update the heading text
    ...
    if (listdata) {
      await waitFor(() => {
        ...
        const versionColumn = screen.getByText("Current Version");
        expect(updatedTable).toBeInTheDocument(); // Render the table on tab switch
        expect(versionColumn).toBeInTheDocument(); // Render the table header with the column present updated at for draft screener list
      });
    }
  });
});
