...
describe("Disqualified Result Page", () => {
  test("renders result page with Complete Intake - Not Qualified", () => {
    const statusMetaData = metaData.map((item) => {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const { subStatus, ...rest } = item;
      return rest;
    });
    const subStatusMetaData = metaData.find((status) => status.code === LeadStatus.COMPLETE_INTAKE_DISQUALIFIED)?.subStatus;
    ...
    radioInputs.forEach((radioInput) => {
      ...
      if (radioInput.id === "Complete Intake - Disqualified") {
    ...
    statusMetaData.map((item) => {
      ...
      if (item.code === LeadStatus.COMPLETE_INTAKE_DISQUALIFIED) {
